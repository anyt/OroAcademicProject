...
class AnytBugTrackerBundleInstaller implements Installation
{
    ...
    /**
     * Create anyt_bugtracker_issue table
     *
     * @param Schema $schema
     */
    protected function createAnytBugtrackerIssueTable(Schema $schema)
    {
        $table = $schema->createTable('anyt_bugtracker_issue');
    ...
    /**
     * Create anyt_bugtracker_issue_priority table
     *
     * @param Schema $schema
     */
    protected function createAnytBugtrackerIssuePriorityTable(Schema $schema)
    {
        $table = $schema->createTable('anyt_bugtracker_issue_priority');
        $table->addColumn('id', 'integer', ['autoincrement' => true]);
    ...
    /**
     * Create anyt_bugtracker_issue_resolution table
     *
     * @param Schema $schema
     */
    protected function createAnytBugtrackerIssueResolutionTable(Schema $schema)
    {
        $table = $schema->createTable('anyt_bugtracker_issue_resolution');
        $table->addColumn('id', 'integer', ['autoincrement' => true]);
        $table->addColumn('title', 'string', ['length' => 255]);
        $table->setPrimaryKey(['id']);
    }
    ...
    /**
     * Add anyt_bugtracker_issue foreign keys.
     *
     * @param Schema $schema
     */
    protected function addAnytBugtrackerIssueForeignKeys(Schema $schema)
    {
        $table = $schema->getTable('anyt_bugtracker_issue');
        $table->addForeignKeyConstraint(
            $schema->getTable('anyt_bugtracker_issue_resolution'),
            ['resolution_id'],
            ['id'],
        ...
        $table->addForeignKeyConstraint(
            $schema->getTable('anyt_bugtracker_issue_priority'),
            ['priority_id'],
            ['id'],
            ['onDelete' => null, 'onUpdate' => null]
        );
